...
| 3–4 | `Eval` class + runner — load dataset, iterate, call LLM, run scorers, record usage + latency, persist run JSON to `.prompteval/runs/` | ✅ | 2026-06-26 — `eval/runner.py` (Eval, Example, RunnerResult, ExampleResult, RunResult, run_eval, default_runner, _load_dataset, _call_scorer, _sum_usages); `eval/persistence.py` (save_run, load_run); 33 tests for runner + 8 for persistence |
| 4 | `prompteval run --prompt path --tag name` CLI | ✅ | 2026-06-26 — discovers Eval via `importlib.util.spec_from_file_location`; `--tag` / `--eval-file` / `--model` flags; per-example progress; 6 CLI tests with mocked OpenAI |
| 5 | `prompteval compare <tag-a> <tag-b>` — paired deltas, bootstrap CIs, p-values, plain-text table | ✅ | 2026-06-26 — `compare/core.py` (pairing, bootstrap CIs, paired t-test, MetricDelta/ComparisonReport); scipy + numpy; CLI subcommand; 26 module tests + 5 CLI tests; 183/183 green |
| 5 | Plain-English verdict + recommendation lines | ✅ | 2026-06-26 — `_make_verdicts`: quality (regressions → "significant regression on X"), cost (% + p-value), recommendation (ship / don't-ship / inconclusive) matches README killer-output |
| 6 | HTML report writer (single static file, no server) | ✅ | 2026-06-26 — `render_html()` in `compare/core.py`; inline styles only; HTML-escaped tag names; color-coded ship/don't-ship badge; `--html PATH` CLI flag; 6 tests |
| 6 | `--fail-on cost+X%,quality-Y%` for CI | ✅ | 2026-06-26 — `compare/gates.py` (parse_gate_spec, evaluate_gates, GateClause, GateBreach, GateSpecError); only significant breaches count; `--fail-on` CLI flag exits 1 on breach; 17 tests |
| 6 | GitHub Action template + example workflow | ✅ | 2026-06-26 — `examples/github-actions/prompteval.yml` (baseline-from-main vs candidate-from-PR pattern, HTML artifact upload, gate at cost+10%/quality-5%); README pointer updated |
...
| 2026-06-26 | Run persistence as `<tag>.json`, latest-wins-on-rerun | Simple identity. Re-running with the same tag overwrites. If you want history, use unique tags (`baseline-v1`, `baseline-v2`) and rely on git. Filename sanitisation strips path separators — defence in depth against malicious tags trying to escape the runs dir. |
| 2026-06-26 | Eval discovery via `importlib.util.spec_from_file_location` + first-`Eval`-instance | Standard Python pattern. We add the eval file's parent dir to sys.path so relative imports inside the user's eval.py work — matching how `python evals/eval.py` would behave. Import errors and missing-Eval-instance both surface as friendly `ClickException`s, not stack traces. |
| 2026-06-26 | scipy + numpy for compare-module stats (bootstrap CIs via `scipy.stats.bootstrap`, paired p-values via `scipy.stats.ttest_rel`) | Considered hand-rolling, but scipy's bootstrap is well-tested (BCa method available later if needed), handles edge cases (single-sample, NaN), and ships with Python's scientific stack — most eval-tool users already have it. Cost: ~30MB of deps. Worth it for correctness + future-proofing (Wilcoxon, BCa, etc. all in scipy when we want them). |
| 2026-06-26 | Bootstrap method = percentile, 10K resamples, seeded RNG (seed=42) | Percentile is the simplest and works fine for our sample sizes (typically 20–200 examples). Seeded RNG = reproducible CI bounds across runs, which matters for snapshot-style tests and "did the report actually change?" comparisons. BCa upgrade path noted in module docstring. |
| 2026-06-26 | Custom bootstrap loop for percent-change CIs (cost/latency), not scipy's | scipy.stats.bootstrap's statistic signature wants a single sample; percent-change needs **paired** resampling (same indices into both arrays) and re-summing. Easier to write the 12-line loop than to fight the API. |
| 2026-06-26 | `_paired_p_value` constant-diff edge case: all diffs == 0 → p=1.0; all diffs == k ≠ 0 → p=0.0 | scipy returns NaN when the variance of differences is zero. Real API data never hits this (every call has noise), but synthetic test data does. Constant-zero diffs = no signal at all → p=1.0. Constant-nonzero diffs = every paired example agrees on the exact same delta → that's *perfect* signal, not degeneracy → p=0.0. Branching explicitly keeps the report sensible for both deterministic-pricing tests and real-world runs. |
| 2026-06-26 | `compare` requires ≥2 paired non-errored examples, raises ValueError otherwise | Stats are undefined for 1 paired point (no variance, no CI). Surfaced as a friendly `ClickException` ("Need at least 2 paired examples for a comparison") so users immediately know to either run a larger dataset or check for errored examples masking the pairing. |
| 2026-06-26 | `delta_pct` is `None` for scorer deltas, set for cost/latency | Scores are bounded [0, 1] — percent change from 0.8 → 0.9 (+12.5%) is technically correct but reads weirdly; `+0.10 absolute` is what users actually want to see. Cost and latency are unbounded positive quantities where "37% reduction" is the natural framing. Rendering branches on `delta_pct is None`. |
| 2026-06-26 | Ship/don't-ship recommendation logic = quality-first | Order of checks: (1) any significant scorer regression → don't ship; (2) significant cost reduction + no regression → ship; (3) significant cost increase + no quality improvement → don't ship; (4) else → inconclusive. Cost wins are only celebrated *after* quality is confirmed safe. Matches the wedge: "did this prompt change save money *without losing quality*?" — quality is the constraint, cost is the objective. |
| 2026-06-26 | HTML report = single self-contained file, inline styles only, no JS | The artifact has to survive email, CI download buttons, Slack uploads, and `open ./report.html` — none of which run external script tags reliably. Inline `<style>` keeps the file portable and ~3KB. Trade-off: no interactive sort/filter; v1.x can add that with a Vue/htmx layer once we know what users actually want to interact with. Color-coded ship/don't-ship badge picks `.ship` / `.dont-ship` / `.inconclusive` from the recommendation string's leading verb. |
| 2026-06-26 | HTML escapes tag names (and all user-controlled strings) via local `_h()` helper, not stdlib `html.escape` | Same five-entity behavior, no extra import — and an explicit reminder in the diff that any rendered string must pass through it. Tag names come from `--tag <name>` on the CLI, which is user-controlled today and CI-config-controlled tomorrow. Reports may be shared, viewed in webmail, or proxied through dashboards; an unescaped `<script>` tag in a report is exactly the sort of bug that doesn't surface in single-developer use and then bites the first team that adopts it. |
| 2026-06-26 | `--fail-on` only counts statistically significant breaches | A 12% cost spike with p=0.3 is exactly the kind of noise that makes flaky CI useless. The whole reason we run paired bootstrap + t-test is so the gate can distinguish signal from noise. If users want a strict "any cost increase fails" gate they can compute it themselves from the JSON report; v1 stays opinionated on significance. |
| 2026-06-26 | `quality-Y%` interpreted as Y/100 absolute drop in scorer mean | Scorers are bounded [0, 1]. "5% quality loss" colloquially means "0.05 absolute drop" — relative percent change is fragile near zero (a 0.01 → 0.005 drop is "50% loss" but probably noise). Tested explicitly: `quality-5%` does not breach on a 0.02 absolute drop but does on a 0.40 drop. |
| 2026-06-26 | Gate parse errors fail fast (before running compute_comparison) | Parsing `--fail-on` happens up-front in the CLI handler. If a user typos `speed-10%` we surface the error in <50ms instead of after the bootstrap loop. Cheap UX win. |
| 2026-06-26 | GitHub Actions example uses `baseline-from-main vs candidate-from-PR` pattern | The natural workflow: every PR compares its prompt against `main`'s prompt. Workflow does `git show origin/main:evals/prompts/v1.txt > /tmp/baseline.txt` then runs both. Alternative considered: persisted runs in a branch / commits. Rejected because (a) inflates repo size, (b) ties run results to git history in a way that's hard to invalidate, (c) the run-from-main approach is stateless and re-runnable. |
| 2026-06-26 | `if: always()` on HTML artifact upload | The HTML report is most valuable when the gate *fails* — that's when you most need a shareable view of what regressed. Default `if: success()` would hide it on the only run that matters. |
...
The kill date is 2026-08-16. **7 working weeks remaining as of 2026-06-26 (Week 6 ✅, Week 7 next — dogfood against mcp-multi-db).** Status colors should march top-to-bottom; if a 📋 below the line you're working on starts looking impossible, defer it explicitly rather than letting it slip silently.
